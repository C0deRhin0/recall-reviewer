...
const baseline = await readJson("security-plus-v7-term-coverage-baseline.json");
const coverageRelease = {
  ...baseline,
  exam_code: "COMPTIA-SECURITY-PLUS",
  edition: "SY0-701-v7",
  release_label: "Google CySec → Security+ V7 coverage.1",
  categories,
  questions: baseline.questions.map((question) => ({
    ...question,
    prompt: `${question.prompt} Source context: ${question.source_reference}`,
  })),
};
...
await Promise.all([
  writeFile(
    join(importDirectory, "google-cysec-security-plus-v7-term-coverage.json"),
    JSON.stringify(coverageRelease, null, 2) + "\n",
    { mode: 0o600 },
  ),
  writeFile(
...
console.log(
  JSON.stringify({
    coverage_questions: coverageRelease.questions.length,
